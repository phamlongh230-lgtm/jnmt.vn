          ...
          {subjects.map((s) => {
            ...
            return (
              <div key={s.id} style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
                <input value={s.name} onChange={(e) => update(s.id, "name", e.target.value)} placeholder={t(lang, "subject_placeholder")} style={{ flex: 2, padding: "0.6rem 0.75rem", border: `1px solid ${border}`, borderRadius: 8, background: inputBg, color: textCol, fontSize: "0.9rem", outline: "none" }} />
                <input type="number" value={s.score} onChange={(e) => update(s.id, "score", e.target.value)} placeholder={t(lang, "score_placeholder")} min={0} max={100} style={{ flex: 1, padding: "0.6rem 0.75rem", border: `1px solid ${g ? g.color : border}`, borderRadius: 8, background: inputBg, color: g ? g.color : textCol, fontSize: "0.9rem", fontWeight: 700, outline: "none" }} />
                <input type="number" value={s.credit} onChange={(e) => update(s.id, "credit", e.target.value)} placeholder={t(lang, "credits_ph")} min={1} max={10} style={{ width: 52, padding: "0.6rem 0.5rem", border: `1px solid ${border}`, borderRadius: 8, background: inputBg, color: textCol, fontSize: "0.9rem", outline: "none", textAlign: "center" }} />
