...
export default function AnnouncementsPage() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (r.ok) { showToast("Đã đăng thông báo!", "success"); setTitle(""); setContent(""); setIsPinned(false); setShowForm(false); load(); }
  ...
  return (
    <div style={{ maxWidth: 700, margin: "0 auto", padding: "1.5rem 1rem" }} className="animate-fade-in">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.25rem", flexWrap: "wrap", gap: "0.75rem" }}>
        <div>
          <h1 style={{ fontSize: "1.4rem", fontWeight: 900, color: "#2563eb", margin: 0 }}>📢 Bảng thông báo</h1>
          <p style={{ color: text2, fontSize: "0.82rem", margin: 0, marginTop: 2 }}>Thông báo từ Ban giám hiệu & Admin</p>
        </div>
        {isAdmin && (
      ...
      {showForm && isAdmin && (
        <div style={{ background: cardBg, border: `1px solid ${border}`, borderRadius: 12, padding: "1.25rem", marginBottom: "1rem" }}>
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Tiêu đề thông báo..." style={{ width: "100%", padding: "0.7rem", border: `1px solid ${border}`, borderRadius: 8, background: inputBg, color: textCol, fontSize: "0.95rem", marginBottom: "0.75rem", outline: "none", boxSizing: "border-box" }} />
          <textarea value={content} onChange={(e) => setContent(e.target.value)} placeholder="Nội dung thông báo..." rows={4} style={{ width: "100%", padding: "0.7rem", border: `1px solid ${border}`, borderRadius: 8, background: inputBg, color: textCol, fontSize: "0.9rem", resize: "vertical", outline: "none", boxSizing: "border-box", marginBottom: "0.75rem" }} />
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <label style={{ display: "flex", alignItems: "center", gap: "0.4rem", fontSize: "0.85rem", color: textCol, cursor: "pointer" }}>
              <input type="checkbox" checked={isPinned} onChange={(e) => setIsPinned(e.target.checked)} /> Ghim lên đầu
            </label>
      ...
      {loading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
          {items.map((item) => (
            <div key={item.id} style={{ background: cardBg, border: `1px solid ${item.isPinned ? "#2563eb" : border}`, borderRadius: 12, overflow: "hidden", borderLeft: item.isPinned ? "4px solid #2563eb" : undefined }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "0.5rem" }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.25rem" }}>
                      {item.isPinned && <span style={{ fontSize: "0.65rem", background: "#2563eb", color: "white", padding: "0.15rem 0.4rem", borderRadius: 4, fontWeight: 700 }}>📌 GHim</span>}
                  ...
                  <div style={{ display: "flex", gap: "0.4rem", alignItems: "center" }}>
                    {isAdmin && (
                      <>
                        <button onClick={(e) => { e.stopPropagation(); pin(item.id); }} title="Ghim/bỏ ghim" style={{ background: "none", border: "none", cursor: "pointer", fontSize: "0.9rem", opacity: 0.6 }}>📌</button>
                        <button onClick={(e) => { e.stopPropagation(); del(item.id); }} title="Xóa" style={{ background: "none", border: "none", cursor: "pointer", color: "#ef4444", fontSize: "0.9rem" }}>🗑️</button>
                      </>
                    )}
