...
|-------|------|----------------|
| `context-synthesizer` | Context compression | Every 10 commits (auto), or /brain-dump |
...
| `/handoff [topic]` | Write a handoff document to `docs/handoff/` so another session, agent, or teammate can resume the work cleanly |
...
| `gitnexus` | Code-intelligence knowledge graph (call chains, blast radius, augmented search) | All agents that read or modify code — mandatory pre-implementation query |
...
GitNexus indexes the codebase into a knowledge graph and exposes it via MCP. **All agents that touch code must query GitNexus before implementing.**
...
1. `gitnexus://repo/{name}/context` — verify index is fresh
2. `gitnexus query <concept>` — understand existing structure, call chains, clusters
3. `gitnexus impact <symbol>` — check blast radius before changing anything

### Available GitNexus Skills
Skills in `.claude/skills/gitnexus/` — read before starting the relevant task:
...
| CLI — index, status, clean, wiki | `gitnexus-cli` |

### Hooks (automatic)
- **PreToolUse** (Grep/Glob/Bash): auto-enriches searches with graph context
- **PostToolUse** (Bash): warns if index is stale after `git commit` / `git merge`

### Index Freshness
If any tool reports the index is stale: run `npx gitnexus analyze` before continuing.
