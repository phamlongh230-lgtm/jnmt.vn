...
Install these once per machine before using the template:

| Tool | Why | Install |
|------|-----|---------|
| **Node.js ≥ 18** | Runs GitNexus, MCP servers, and the graph-context hook | [nodejs.org](https://nodejs.org) or `nvm install 18` |
| **`jq`** | Required by `guard-destructive.sh` — the hook **fails closed** (blocks every Bash call) if `jq` is missing, so skipping this turns Claude into a paperweight | macOS `brew install jq` · Debian/Ubuntu `sudo apt-get install jq` · Windows `winget install jqlang.jq` |
| **`gh` CLI** *(optional)* | Lets agents open PRs, read issues, check CI status | [cli.github.com](https://cli.github.com) — then `gh auth login` |
| **GitNexus** *(auto)* | Code-intelligence knowledge graph | Auto-installed via `npx` on first use — no action needed |
...
├── .mcp.json                     # MCP server config (sequential-thinking, context7, gitnexus)
├── .gitignore
│
├── .claude/
...
│   │   └── gitnexus-hook.js      # Pre/PostToolUse: enriches searches with graph context, warns on stale index
...
│   ├── skills/                   # GitNexus specialist skills — loaded automatically by Claude Code
│   │   └── gitnexus/             # exploring, impact-analysis, debugging, refactoring, pr-review, cli, guide
...
This template includes [GitNexus](https://github.com/nicosxt/gitnexus) — a code intelligence layer that indexes your codebase into a knowledge graph and feeds it to agents via MCP.

### Why it's here
Claude Code agents "ngáo" (get confused) because they don't understand the codebase structure — they miss dependencies, break call chains, and make blind edits. GitNexus fixes this by giving every agent a full architectural view before they touch anything.

### First-time setup
```bash
# After cloning your project
npx gitnexus analyze
```

Then run `/start` in Claude Code as usual.

### What's added
- **`.mcp.json`** — GitNexus MCP server alongside `sequential-thinking` and `context7`
- **`.claude/skills/gitnexus/`** — 6 specialist skills (exploring, debugging, impact analysis, refactoring, PR review, CLI)
- **`.claude/hooks/gitnexus-hook.js`** — Hook that auto-enriches every Grep/Glob/Bash search with graph context
- **`.claude/settings.json`** — GitNexus hooks merged with existing template hooks
- **`CLAUDE.md`** — GitNexus workflow added to agent instructions
- **Agents updated** — `systems-architect`, `backend-developer`, `frontend-developer`, `database-expert`, `qa-engineer` all query GitNexus before implementing
